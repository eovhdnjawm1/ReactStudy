...
function App() {
  ...
  return (
    <div className="App">
      ...
      <button onClick={() => {titleChange(0)}}>글제목0</button>
      <button onClick={() => {titleChange(1)}}>글제목1</button>
      <button onClick={() => {titleChange(2)}}>글제목2</button>
            {
        // 조건문을 쓸려면 if 는 안되니깐 삼항 연산자를 써야한다.
        modal === true ? <TitleModal title={title} colorText="오렌쥐" color={'black'} 글제목={글제목} b={b} /> : null
      }
    </div>
  );
}
...
function TitleModal(props) {
  const title = ['남자코트 추천', '강남 우동맛집', '파이썬독학']
  return (
